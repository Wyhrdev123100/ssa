...
// ===== Setup =====
// Local dùng ./uploads, Railway dùng /data/uploads (set UPLOAD_DIR trong env)
...
try {
  ...
  console.error(`❌ Không tạo được thư mục: ${err.message}`);
  process.exit(1);
}
...
function startStream(videoPath, streamKey) {
  ...
  addLog(`📡 RTMP URL: ${rtmpUrl.replace(streamKey, "****")}`);
  ...
  ffmpegProcess.stderr.on("data", (data) => {
    ...
    if (text.includes("Error") || text.includes("error")) {
      addLog(`❌ ${text.trim().substring(0, 120)}`);
    }
  });

  ffmpegProcess.on("close", (code) => {
    addLog(`⚠️ FFmpeg kết thúc (code: ${code})`);
    isStreaming = false;
    ffmpegProcess = null;
  });

  ffmpegProcess.on("error", (err) => {
    addLog(`❌ Lỗi FFmpeg: ${err.message}`);
    isStreaming = false;
  });
}
...
app.post("/upload", upload.single("video"), (req, res) => {
  ...
  addLog(
    `✅ Đã upload: ${req.file.originalname} (${(req.file.size / 1024 / 1024).toFixed(1)} MB)`,
  );
  res.json({
    success: true,
...
app.post("/start", (req, res) => {
  ...
  if (!currentVideo) {
    return res.json({
      success: false,
      message: "⚠️ Chưa có video! Hãy upload video trước.",
    });
  }
  if (!streamKey || streamKey.trim() === "") {
    return res.json({ success: false, message: "⚠️ Chưa nhập Stream Key!" });
  }
  if (!fs.existsSync(currentVideo)) {
    currentVideo = null;
    return res.json({
      success: false,
      message: "⚠️ File video không tồn tại! Hãy upload lại.",
    });
  }
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  addLog(`🚀 Server khởi động - Port: ${PORT}`);
});
